...
class FirefoxHandler:
    def __init__(self, headless=False, private_mode=True, disable_cache=True, block_cookies=True, proxy=None):
        self.gecko_path = os.path.join(
            os.environ['SystemRoot'], 'geckodriver.exe')
        self.binary_path = os.path.normpath(os.path.join(
            os.environ['ProgramFiles'], 'Mozilla Firefox', 'firefox.exe'))
        self.firefox_service = FirefoxService(executable_path=self.gecko_path)
        self.binary_location = self.binary_path
        self.options = FirefoxOptions()
        self.options.set_preference(
            "browser.privatebrowsing.autostart", private_mode)
        self.options.set_preference(
            "browser.cache.disk.enable", not disable_cache)
        self.options.set_preference(
            "browser.cache.memory.enable", not disable_cache)
        self.options.set_preference(
            "network.cookie.cookieBehavior", block_cookies)
        if proxy:
            self.options.proxy = Proxy(
                {'proxyType': 'manual', 'httpProxy': proxy, 'sslProxy': proxy})
